...
public class WorkerExecutionService {
    ...
    public TaskProgress progress(String tenantId, UUID taskId, WorkerProgressRequest request) {
        Task task = require(tenantId, taskId);
        if (!leases.require(taskId).matches(request.workerId(), request.attempt(), request.leaseToken())) throw new IllegalStateException("invalid lease");
    ...
    public TaskArtifact artifact(String tenantId, UUID taskId, String workerId, int attempt, String token, String key, String contentType, long size, String checksum) {
        Task task = require(tenantId, taskId);
        if (!leases.require(taskId).matches(workerId, attempt, token) || (task.getStatus() != TaskStatus.DISPATCHED && task.getStatus() != TaskStatus.RUNNING)) throw new IllegalStateException("invalid lease");
