...
public class WorkerExecutionController {
    ...
    public WorkerExecutionController(WorkerExecutionService service, TaskProgressService progress, TaskLogService logs, TaskArtifactService artifacts, TaskQueryService queries) {
        this.service = service; this.progress = progress; this.logs = logs; this.artifacts = artifacts; this.queries = queries;
    }
    ...
    public record ArtifactRequest(@NotBlank String workerId, int attempt, @NotBlank String leaseToken, @NotBlank String objectKey, @NotBlank String contentType, long size, String checksum) {}
    ...
    public TaskArtifact artifact(@RequestHeader("X-Tenant-Id") String tenantId, @PathVariable UUID taskId, @Valid @RequestBody ArtifactRequest req) {
        return service.artifact(tenantId, taskId, req.workerId(), req.attempt(), req.leaseToken(), req.objectKey(), req.contentType(), req.size(), req.checksum());
    }
    @GetMapping("/{taskId}/progress")
    public TaskProgress progress(@RequestHeader("X-Tenant-Id") String tenantId, @PathVariable UUID taskId) { queries.require(tenantId, taskId); return progress.get(taskId); }
    @GetMapping("/{taskId}/logs")
    public java.util.List<TaskLogEntry> logs(@RequestHeader("X-Tenant-Id") String tenantId, @PathVariable UUID taskId, @RequestParam(required = false) String level) { queries.require(tenantId, taskId); return logs.list(taskId, level); }
    @GetMapping("/{taskId}/artifacts")
    public java.util.List<TaskArtifact> artifacts(@RequestHeader("X-Tenant-Id") String tenantId, @PathVariable UUID taskId) { queries.require(tenantId, taskId); return artifacts.list(taskId); }
}
